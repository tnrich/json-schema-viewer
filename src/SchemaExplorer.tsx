    ...
    <BreadcrumbsItem
      ...
      component={() => (
        <NavLinkPreservingSearch
          ...
          exact={true}
        >
          {getTitle(pe.reference, {
            title: pe.title !== "object" ? pe.title : undefined,
          })}
        </NavLinkPreservingSearch>
      )}
    />
    ...
    return (
      <>
        <a href="#" onClick={onClick}>
          {this.state.open ? this.props.onOpen : this.props.onClosed}
        </a>
...
const SchemaExplorerExample: React.FC<SchemaExplorerExampleProps> = (props) => {
  const potentialExample = generateJsonExampleFor(
    props.schema,
    props.lookup,
    props.stage
  );
  ...
  if (isExample(potentialExample)) {
    const renderedOutput =
      props.format === "json"
        ? JSON.stringify(potentialExample.value, null, 2)
        : dump(potentialExample.value);
    return (
      <FullWidth>
  ...
  return (
    <div>
      ...
      <Expand
        onOpen="(Collapse advanced view)"
        onClosed="(Expand advanced view)"
      >
        <div>
...
export const SchemaExplorerDetails: React.FC<SchemaExplorerDetailsProps> = (
  props
) => {
...
function createClickElement(details: JsonSchemaObjectClickProps): ClickElement {
  return (props) => {
    if (isExternalReference(props.schema) && props.schema.$ref !== undefined) {
      const externalUrl = externalLinkTo(
        details.basePathSegments,
        props.schema.$ref
      );
      if (externalUrl === null) {
        return <Anything />;
    ...
    const references = [
      ...details.path.map((p) => p.reference),
      props.reference,
    ];
    return (
...
export class SchemaExplorer extends React.PureComponent<
  SchemaExplorerProps,
  SchemaExplorerState
> {
  ...
  render() {
    ...
    if (path.length === 0) {
      return (
        <div>
          TODO What do we do when the reference could not be found? Error maybe?
        </div>
      );
    }
    ...
    return (
      <SchemaExplorer.Container>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          {schema.$id && (
            <a style={{ fontSize: 11, marginLeft: 10 }} href={schema.$id}>
              View Raw
            </a>
          )}
        </div>
