...
function isLoadSchemaError(
  e: JsonSchema | LoadSchemaError
): e is LoadSchemaError {
  return typeof e !== "boolean" && "message" in e;
}

class LoadSchemaWR extends React.PureComponent<
  LoadSchemaProps,
  LoadSchemaState
> {
  ...
  componentDidUpdate(prevProps: LoadSchemaProps, prevState: LoadSchemaState) {
    ...
    if (
      ...
      prevState.result.currentUrl !== url &&
      url !== null
    ) {
      this.loadUrl(url);
    }
  ...
  componentDidMount() {
    const url = this.getUrlFromProps();
    if (url !== null && !localStorage.getItem("jsonInput")) {
      this.loadUrl(url);
    } else {
      this.loadJson();
    }
  ...
  private loadUrl(url: string): void {
    fetch(url)
      .then((resp) => resp.json())
      .then((schema) => {
        return this.setState({ result: { schema, currentUrl: url } });
      })
      .catch((e) =>
        this.setState({
          result: { currentUrl: url, schema: { message: e.message } },
        })
      );
  }
  private loadJson(): void {
    if (localStorage.getItem("jsonInput")) {
      const schema = JSON.parse(localStorage.getItem("jsonInput") || "");
      this.setState({ result: { schema, currentUrl: "localStorage" } });
    }
  }
  ...
  render() {
    ...
    if (result === undefined) {
      return (
        <EmptyState
          ...
          primaryAction={<Spinner size="xlarge" />}
        />
      );
    }
    ...
    if (isLoadSchemaError(result.schema)) {
      return (
        <EmptyState
          ...
          primaryAction={<p>Error: ${result.schema.message}</p>}
        />
      );
    }
    ...
    const linkTitle =
      typeof result.schema !== "boolean"
        ? result.schema.title || result.currentUrl
        : result.currentUrl;
...
export const LoadSchema = withRouter<LoadSchemaProps, typeof LoadSchemaWR>(
  LoadSchemaWR
);
